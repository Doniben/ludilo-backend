...
def list_songs(req: func.HttpRequest) -> func.HttpResponse:
    ...
    query = "SELECT c.id, c.title, c.status, c.format, c.createdAt, c.stems, c.midiFiles, c.progress, c.originalBlobPath, c.chords, c.source, c.librarySource, c.originalUserId, c.transcription_model FROM c WHERE c.userId = @userId ORDER BY c.createdAt DESC"
